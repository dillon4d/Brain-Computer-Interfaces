...
%
%
VAR_NAME = "Dillon Ford"; %" rather than ' for apostrophe last names
VAR_PID  = 'A16092047';

%--------------------------------------------------------------------------
% Q1 -- What are the four classes present in this dataset?
% Change the string to your answer.
Q1_ANS = 'Cue onset left, right, foot, and tongue.'; % 
...
Q3_Ans = [size(EEGR_train{1, 1})]; % Put your answer here
...
i = 1;
for i = 1:k
    valIdx{i} = randIdx(1:bin_size);
    %celldisp(valIdx)
    trainIdx{i} = randIdx(bin_size+1:end);
    %celldisp(trainIdx)
    %shift all row indices by 2
    randIdx = circshift(randIdx,2, 2);
end
